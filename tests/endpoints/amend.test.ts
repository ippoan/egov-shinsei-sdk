...
describe('amendApplication', () => {
  ...
  it('POST /apply/amend submits amendment', async () => {
    server.use(
      http.post(`${API_BASE}/apply/amend`, () => {
        return HttpResponse.json({
          metadata: { title: 'ok', detail: '', type: '', instance: '' },
          results: { arrive_id: 'A001', amend_date: '2024-01-01 00:00:00' },
        });
      }),
    );
    const res = await client.amendApplication({
      arrive_id: 'A001',
      send_file: { file_name: 'amend.zip', file_data: 'base64' },
    });
  ...
  it('throws on error', async () => {
    ...
    await expect(
      client.amendApplication({ arrive_id: 'A', send_file: { file_name: 'f', file_data: '' } }),
